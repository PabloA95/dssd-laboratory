...
class ProtocolsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create protocol" do
    assert_difference('Protocol.count') do
      post protocols_url, params: { protocol: { local: @protocol.local, name: @protocol.name } }
    end
  ...
  test "should update protocol" do
    patch protocol_url(@protocol), params: { protocol: { local: @protocol.local, name: @protocol.name } }
